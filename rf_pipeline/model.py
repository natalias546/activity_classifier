...
print("Subjects — train:", len(train_subs), "val:", len(val_subs), "test:", len(test_subs))
...
print("Watch RF val accuracy:", builtins.round(evaluator.evaluate(watch_val_preds), 4))
print("Phone RF val accuracy:", builtins.round(evaluator.evaluate(phone_val_preds), 4))
...
def per_activity_accuracy(preds, name):
    print(name, "per-activity accuracy:")
    rows = (
        preds
    ...
    for r in rows:
        ...
        print(r.activity_label, name_str, builtins.round(r.accuracy, 4), r.windows)
...
def print_feature_importance(model, feat_cols, name, top_n=15):
    ...
    print(name, "top", top_n, "feature importances:")
    for feat, imp in pairs[:top_n]:
        print(feat, builtins.round(imp, 4))
...
print("Stacked model test accuracy:", builtins.round(evaluator.evaluate(test_final), 4))

# Per-activity accuracy
# map numeric label back to activity name using StringIndexer labels
labels = watch_model.stages[0].labels  # index to activity letter
